...
class AppController(QObject):
    ...
    subtitle_quality_filter_requested = Signal(object)
    subtitle_interjection_cleanup_requested = Signal()
    ...
    def request_subtitle_quality_filter(self, kind: str | None) -> None:
        """Request subtitle inspector to apply a quality filter."""

        normalized = kind if kind in {"ocr", "speed", "duplicate"} else None
        self.subtitle_quality_filter_requested.emit(normalized)

    def request_subtitle_interjection_cleanup(self) -> None:
        """Request subtitle inspector to open the interjection cleanup flow."""

        self.subtitle_interjection_cleanup_requested.emit()
