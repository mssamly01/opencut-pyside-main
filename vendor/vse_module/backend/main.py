...
try:
    ...
    if 'attempted relative import with no known parent package' not in str(e):
        raise
    # Fallback khi chạy file trực tiếp theo kiểu script.
...
import threading
import platform
# import multiprocessing  # Tắt multiprocessing hoàn toàn
import time
import pysrt
...
class SubtitleDetect:
    ...
    def __init__(self):
        ...
        import os
        import paddleocr
        # Add paddleocr directory to sys.path so it can find its internal ppocr module
        # Handle cases where paddleocr is a namespace package (no __file__)
        if hasattr(paddleocr, '__path__'):
            paddleocr_dir = paddleocr.__path__[0]
        else:
            paddleocr_dir = os.path.dirname(paddleocr.__file__) if paddleocr.__file__ else None
            
        if paddleocr_dir and paddleocr_dir not in sys.path:
...
class SubtitleExtractor:
    ...
    def __init__(self, vd_path, sub_area=None):
        ...
        self.video_cap = cv2.VideoCapture(vd_path, cv2.CAP_FFMPEG)
        # 通过视频路径获取视频名称
        ...
        # 若目录不存在，则创建文件夹
        print(f"📁 Tạo thư mục frame output: {self.frame_output_dir}")
        if not os.path.exists(self.frame_output_dir):
            os.makedirs(self.frame_output_dir)
            print(f"✅ Đã tạo thư mục frame output")
        else:
            print(f"ℹ️ Thư mục frame output đã tồn tại")
            
        print(f"📁 Tạo thư mục subtitle output: {self.subtitle_output_dir}")
        if not os.path.exists(self.subtitle_output_dir):
            os.makedirs(self.subtitle_output_dir)
            print(f"✅ Đã tạo thư mục subtitle output")
        else:
            print(f"ℹ️ Thư mục subtitle output đã tồn tại")
        # 定义是否使用vsf提取字幕帧
        self.use_vsf = False
        # 定义vsf的字幕输出路径
        ...
        print(f"📄 Đường dẫn raw subtitle sẽ là: {self.raw_subtitle_path}")
        # 自定义ocr对象
        self.ocr = None
        # 打印识别语言与识别模式
        ...
        # 图像处理回调函数
        self.image_processing_callback = None

    def set_image_processing_callback(self, callback):
        """设置图像处理回调函数"""
        self.image_processing_callback = callback
    

    def run(self):
        """
        运行整个提取视频的步骤 - 优化后的工作流程
        """
        # 记录开始运行的时间
        ...
        # === BƯỚC 1: TRÍCH XUẤT ẢNH TỪ VIDEO ===
        print(config.interface_config['Main']['StartProcessFrame'])
        print("Bước 1: Trích xuất ảnh từ video bằng VideoSubFinder...")
        
        # Khởi tạo OCR queue trước khi VSF chạy
        from multiprocessing import Queue
        self.subtitle_ocr_task_queue = Queue()
        self.subtitle_ocr_progress_queue = Queue()
        
        if self.sub_area is not None:
            if platform.system() in ['Windows', 'Linux']:
                # Luôn sử dụng VSF để đảm bảo có RGBImages và ISAImages
        ...
        # === BƯỚC 2: GỘP ẢNH RGB-ISA NÂNG CAO (NẾU ĐƯỢC BẬT) ===
        if self.image_processing_callback:
            print("Bước 2: Gộp ảnh RGB-ISA nâng cao...")
            rgb_images_dir = os.path.join(self.temp_output_dir, 'RGBImages')
            isa_images_dir = os.path.join(self.temp_output_dir, 'ISAImages')
            
            if self.image_processing_callback(rgb_images_dir, isa_images_dir):
                print("Gộp ảnh hoàn tất. OCR sẽ sử dụng ảnh đã gộp.")
            else:
                print("Có lỗi trong quá trình gộp ảnh. OCR sẽ sử dụng ảnh gốc.")
        else:
            print("Bước 2: Bỏ qua gộp ảnh RGB-ISA nâng cao.")

        # === BƯỚC 3: OCR NHẬN DẠNG VĂN BẢN ===
        print("Bước 3: Bắt đầu OCR nhận dạng văn bản...")
        ...
        # Kiểm tra file SRT có được tạo ra không
        srt_file_path = os.path.join(os.path.splitext(self.video_path)[0] + '.srt')
        if os.path.exists(srt_file_path):
            print(f"✅ File SRT đã được tạo thành công: {srt_file_path}")
            
        else:
            print(f"❌ File SRT không được tạo ra: {srt_file_path}")
            print("Có thể có lỗi trong quá trình xử lý subtitle.")
        if config.WORD_SEGMENTATION:
            srt_file_path = os.path.join(os.path.splitext(self.video_path)[0] + '.srt')
            if os.path.exists(srt_file_path):
                try:
                    reformat.execute(srt_file_path, config.REC_CHAR_TYPE)
                    print(f"✅ Đã thực hiện word segmentation cho file: {srt_file_path}")
                except Exception as e:
                    print(f"⚠️ Lỗi khi thực hiện word segmentation: {e}")
            else:
                print(f"⚠️ File SRT không tồn tại để thực hiện word segmentation: {srt_file_path}")
    ...
    def extract_frame_by_vsf(self):
       ...
       """
        ...
        def count_process():
            ...
            last_total_ms = -1  # Thay đổi từ 0 thành -1 để xử lý ảnh đầu tiên có total_ms = 0
            processed_image = set()
            ...
            while self.vsf_running and not self.isFinished:
                ...
                try:
                    ...
                    for rgb_image in rgb_images:
                        ...
                        if total_ms > last_total_ms:  # Bây giờ total_ms = 0 sẽ > -1
                            # Sử dụng cùng logic với _timestamp_to_frameno để đảm bảo nhất quán
                            frame_no = self._timestamp_to_frameno(total_ms)
                            task = (self.frame_count, frame_no, None, None, total_ms, self.default_subtitle_area)
                            self.subtitle_ocr_task_queue.put(task)
                            # print(f"📤 Đã gửi task cho ảnh {rgb_image}: frame_no={frame_no}, total_ms={total_ms}")
        ...
        # Tổng thời lượng video (ms), dùng cho cập nhật tiến độ VSF->OCR.
        duration_ms = (self.frame_count / self.fps) * 1000 if self.fps > 0 else 0
        if config.USE_GPU and len(config.ONNX_PROVIDERS) > 0:
            cpu_count = 4  # Sử dụng số CPU cố định thay vì multiprocessing
        else:
            cpu_count = 4  # Sử dụng số CPU cố định thay vì multiprocessing
            if cpu_count < 4:
                cpu_count = 4  # Sử dụng số CPU cố định thay vì multiprocessing
        if platform.system() == 'Windows':
            # 定义执行命令
            vsf_dir = os.path.dirname(path_vsf)
            # -ovffmpeg: Sử dụng FFMPEG để mở video (tốt cho GPU)
            # --use_cuda: Sử dụng CUDA để xử lý ảnh
            # /hw_device:cuda: Chỉ định rõ thiết bị phần cứng là CUDA
            cmd = f'"{path_vsf}" --use_cuda -ovffmpeg /hw_device:cuda -c -r -i "{self.video_path}" -o "{self.temp_output_dir}" -ces "{self.vsf_subtitle}" '
            cmd += f"-te {top_end} -be {bottom_end} -le {left_end} -re {right_end} -nthr {cpu_count} -nocrthr {cpu_count}"
            self.vsf_running = True
            import subprocess
            print(f"🚀 Đang khởi chạy VSF với GPU (cwd: {vsf_dir})...")
            completed = subprocess.run(cmd, shell=True, cwd=vsf_dir, capture_output=True, text=True, encoding='utf-8', errors='ignore')
            self.vsf_running = False

            if completed.returncode != 0:
                print(f"⚠️ VSF thoát với mã lỗi: {completed.returncode}")
                if completed.stdout:
                    print(completed.stdout[-2000:])
                if completed.stderr:
                    print(completed.stderr[-2000:])

            # Đưa toàn bộ ảnh VSF vào OCR queue theo cách đồng bộ để tránh race condition.
            rgb_images_path = os.path.join(self.temp_output_dir, 'RGBImages')
            if not os.path.exists(rgb_images_path):
                print(f"❌ Không tìm thấy thư mục RGBImages sau khi VSF chạy: {rgb_images_path}")
                return

            rgb_images = sorted(os.listdir(rgb_images_path))
            print(f"🖼️ VSF đã tạo {len(rgb_images)} ảnh trong RGBImages")
            if len(rgb_images) == 0:
                return
            ...
            for rgb_image in rgb_images:
                ...
                if duration_ms > 0:
                    progress = min(100, (total_ms / duration_ms) * 100)
                    self.update_progress(frame_extract=progress)

            self.update_progress(frame_extract=100)
            
            # Xử lý ảnh sẽ được thực hiện trong quy trình chính sau khi VSF hoàn thành
        else:
            # 定义执行命令
            vsf_dir = os.path.dirname(path_vsf)
            cmd = f'"{path_vsf}" -c -r -i "{self.video_path}" -o "{self.temp_output_dir}" -ces "{self.vsf_subtitle}" '
            if config.USE_GPU:
                # -ovffmpeg and /hw_device:cuda might also work on Linux CLI version
                cmd += "--use_cuda -ovffmpeg /hw_device:cuda "
            cmd += f"-te {top_end} -be {bottom_end} -le {left_end} -re {right_end} -nthr {cpu_count} -dsi"
            self.vsf_running = True
            import subprocess
            print(f"🚀 Đang khởi chạy VSF (Linux) (cwd: {vsf_dir})...")
            p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, bufsize=1,
                                 close_fds='posix' in sys.builtin_module_names, shell=True, cwd=vsf_dir)
    ...
    def generate_subtitle_file(self):
        ...
        if not self.use_vsf:
            ...
            print(f"📝 Có {len(subtitle_content)} subtitle content từ OCR (NO-VSF)")
            
            if not subtitle_content:
                print("❌ Không có subtitle content từ OCR (NO-VSF)!")
                return []
            ...
            try:
                ...
                print(f"✅ File SRT đã được lưu thành công: {srt_filename}")
                # 返回持续时间低于1s的字幕行
                return post_process_subtitle
            except Exception as e:
                print(f"❌ Lỗi khi lưu file SRT (NO-VSF): {e}")
                print(f"Đường dẫn file: {srt_filename}")
                print(f"Số lượng subtitle: {len(subtitle_content)}")
                return []
    ...
    def generate_subtitle_file_vsf(self):
        ...
        # Debug: Kiểm tra file VSF subtitle có tồn tại không
        if not os.path.exists(self.vsf_subtitle):
            print(f"❌ Lỗi: File VSF subtitle không tồn tại: {self.vsf_subtitle}")
            return
        
        try:
            subs = pysrt.open(self.vsf_subtitle)
            print(f"📄 Đọc được {len(subs)} subtitle từ file VSF")
        except Exception as e:
            print(f"❌ Lỗi khi đọc file VSF subtitle: {e}")
            return
        
        sub_no_map = {}
        for sub in subs:
            original_ordinal = sub.start.ordinal
            frame_no = self._timestamp_to_frameno(sub.start.ordinal)
            sub.start.no = frame_no
            sub_no_map[frame_no] = sub

        subtitle_content = self._remove_duplicate_subtitle()
        print(f"📝 Có {len(subtitle_content)} subtitle content từ OCR")
        
        if not subtitle_content:
            print("⚠️ Không có subtitle content từ OCR, dùng fallback từ file VSF gốc")
            srt_filename = os.path.join(os.path.splitext(self.video_path)[0] + '.srt')
            if len(subs) == 0:
                print("❌ File VSF cũng không có dòng phụ đề")
                return
            try:
                fallback_subs = []
                for idx, sub in enumerate(subs, start=1):
                    sub.index = idx
                    fallback_subs.append(sub)
                pysrt.SubRipFile(fallback_subs).save(srt_filename, encoding='utf-8')
                print(f"✅ Đã tạo file SRT fallback từ VSF: {srt_filename}")
            except Exception as e:
                print(f"❌ Không thể lưu file SRT fallback: {e}")
            return
        
        subtitle_content_start_map = {int(a[0]): a for a in subtitle_content}
        print(f"🔍 Subtitle content start map: {list(subtitle_content_start_map.keys())}")
        ...
        print(f"📊 Đã match {matched_count} subtitle từ {len(subs)} VSF subtitle")

        print(f"📝 Tạo được {len(final_subtitles)} subtitle cuối cùng")
        
        if not final_subtitles:
            print("❌ Không có subtitle nào để lưu!")
            return
        ...
        try:
            ...
            print(f"Đường dẫn file: {srt_filename}")
            print(f"Số lượng subtitle: {len(final_subtitles)}")
    ...
    def _detect_watermark_area(self):
        ...
        # 将原txt文件的坐标更新为归一后的坐标
        raw_subtitle_path_normalized = os.path.normpath(self.raw_subtitle_path)
        with open(raw_subtitle_path_normalized, mode='w', encoding='utf-8') as f:
    ...
    def _timestamp_to_frameno(self, time_ms):
        # Chuyển timestamp (ms) sang frame number theo công thức chuẩn: frame = t(s) * fps.
        if self.fps <= 0:
            return 1
        frame_no = max(1, int((time_ms / 1000.0) * self.fps) + 1)
        return frame_no
    ...
    def _remove_duplicate_subtitle(self):
        ...
        # Debug: Kiểm tra file raw subtitle có tồn tại không
        raw_subtitle_path_normalized = os.path.normpath(self.raw_subtitle_path)
        if not os.path.exists(raw_subtitle_path_normalized):
            print(f"❌ Lỗi: File raw subtitle không tồn tại: {raw_subtitle_path_normalized}")
            return []
        
        print(f"📄 Đọc file raw subtitle: {raw_subtitle_path_normalized}")
        with open(raw_subtitle_path_normalized, mode='r', encoding='utf-8') as r:
            lines = r.readlines()
        
        print(f"📄 Đọc được {len(lines)} dòng từ file raw subtitle")
        ...
        for line in lines:
            if line.strip():  # Bỏ qua dòng trống
                parts = line.split('\t')
                if len(parts) >= 3:  # Đảm bảo có đủ 3 phần
                    frame_no = parts[0]
                    content = parts[2]
                    content_list.append(RawInfo(frame_no, content))
                else:
                    print(f"⚠️ Dòng không đúng định dạng: {line.strip()}")
        
        print(f"Tổng số ảnh OCR: {len(content_list)}")
        
        # Đếm số ảnh lỗi từ thư mục InvalidImages
        invalid_images_dir = os.path.join(self.temp_output_dir, 'InvalidImages')
        invalid_count = 0
        if os.path.exists(invalid_images_dir):
            invalid_files = [f for f in os.listdir(invalid_images_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.txt'))]
            invalid_count = len(invalid_files)
            if invalid_count > 0:
                print(f"Số ảnh lỗi được lưu trong InvalidImages: {invalid_count}")
                # Phân loại ảnh lỗi
                error_files = [f for f in invalid_files if f.startswith('error_')]
                empty_files = [f for f in invalid_files if f.startswith('empty_')]
                no_text_files = [f for f in invalid_files if f.startswith('no_text_')]
                invalid_files_other = [f for f in invalid_files if f.startswith('invalid_')]
                
                if error_files:
                    print(f"  - Ảnh lỗi OCR: {len(error_files)}")
                if empty_files:
                    print(f"  - Ảnh trống: {len(empty_files)}")
                if no_text_files:
                    print(f"  - Ảnh không có văn bản: {len(no_text_files)}")
                if invalid_files_other:
                    print(f"  - Ảnh không hợp lệ: {len(invalid_files_other)}")
                ...
                if idx_j + 1 == content_list_len or ratio(i.content.replace(' ', ''), content_list[idx_j + 1].content.replace(' ', '')) < config.THRESHOLD_TEXT_SIMILARITY:
                    ...
                    if not self.use_vsf:
                        if end_frame == start_frame and idx_j + 1 < content_list_len:
                            ...
                            end_frame = content_list[idx_j + 1].no
                    # 寻找最长字幕
        ...
        print(f"Số dòng phụ đề sau khi gộp trùng lặp: {len(unique_subtitle_list)}")
        print(f"Số ảnh bị gộp/loại bỏ: {len(content_list) - len(unique_subtitle_list)}")
        
        # Thống kê tổng hợp
        total_images_processed = len(content_list) + invalid_count
        print(f"📊 Tổng kết: {total_images_processed} ảnh được xử lý ({len(content_list)} OCR thành công + {invalid_count} ảnh lỗi)")
    ...
    def __delete_frame_cache(self):
        if not config.DEBUG_NO_DELETE_CACHE:
            if len(os.listdir(self.frame_output_dir)) > 0:
                print(f"🗑️ Đang xóa cache frames trong: {self.frame_output_dir}")
                for i in os.listdir(self.frame_output_dir):
                    os.remove(os.path.join(self.frame_output_dir, i))
                print(f"✅ Đã xóa cache frames thành công")
    ...
    def empty_cache(self):
        ...
        if not config.DEBUG_NO_DELETE_CACHE:
            if os.path.exists(self.temp_output_dir):
                print(f"🗑️ Đang xóa thư mục cache VSF: {self.temp_output_dir}")
                shutil.rmtree(self.temp_output_dir, True)
                print(f"✅ Đã xóa thành công thư mục cache VSF")
            else:
                print(f"ℹ️ Thư mục cache VSF không tồn tại: {self.temp_output_dir}")
        else:
            print(f"🔧 Chế độ debug: Giữ lại thư mục cache VSF: {self.temp_output_dir}")
    ...
    def start_subtitle_ocr_async(self):
        def get_ocr_progress():
            ...
            while True:
                ...
                if notify:
                    ...
                    notify = False
                # Xử lý trường hợp current_frame_no có thể là array
                try:
                    if hasattr(current_frame_no, '__len__') and len(current_frame_no) > 1:
                        # Nếu là array, lấy phần tử đầu tiên
                        current_frame_no = current_frame_no[0] if len(current_frame_no) > 0 else 0
                    elif hasattr(current_frame_no, '__len__') and len(current_frame_no) == 0:
                        # Nếu là array rỗng, đặt thành 0
                        current_frame_no = 0
                    elif hasattr(current_frame_no, '__len__'):
                        # Nếu có __len__ nhưng không phải array thông thường, thử convert
                        current_frame_no = int(current_frame_no) if current_frame_no.size > 0 else 0
                except (AttributeError, TypeError, ValueError):
                    # Nếu có lỗi, đặt thành 0
                    current_frame_no = 0
                
                # Đảm bảo current_frame_no là số đơn lẻ trước khi sử dụng
                try:
                    if hasattr(current_frame_no, '__len__'):
                        # Nếu vẫn là array, lấy phần tử đầu tiên
                        current_frame_no = current_frame_no[0] if len(current_frame_no) > 0 else 0
                    current_frame_no = int(current_frame_no)
                except (AttributeError, TypeError, ValueError, IndexError):
                    current_frame_no = 0
                
                # Thêm error handling cho comparison
                try:
                    is_complete = (current_frame_no == -1)
                    if is_complete:
                        # Chỉ set progress = 95% và không hiển thị "hoàn thành"
                        progress_percent = 95
                        self.update_progress(ocr=progress_percent)
                        print("🔄 Đang hoàn tất quá trình OCR...")
                        # Không set progress = 100% ở đây, để thread khác xử lý
                        return
                    else:
                        progress_percent = (current_frame_no / total_frame_count * 100)
                        self.update_progress(ocr=progress_percent)
                        
                except Exception as e:
                    print(f"DEBUG: Lỗi khi update progress: {e}")
                    # Fallback: set progress to 100% và return
                    self.update_progress(ocr=100)
                    return

        
        # Lấy danh sách ảnh đã xử lý từ RGBImages để giữ nguyên tên file
        ...
        if os.path.exists(rgb_images_dir):
            ...
            for filename in os.listdir(rgb_images_dir):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                    try:
                        # Từ tên file VSF format: 0_00_00_033__...
                        ...
                        continue
            print(f"📋 Tìm thấy {len(processed_images)} ảnh đã xử lý để giữ nguyên tên file")
        
        # Sử dụng queue đã có thay vì tạo mới
        if self.subtitle_ocr_task_queue is None:
            from multiprocessing import Queue
            self.subtitle_ocr_task_queue = Queue()
            self.subtitle_ocr_progress_queue = Queue()
        ...
        process, task_queue, progress_queue = subtitle_ocr.async_start(self.video_path,
                                                                       ...
                                                                       task_queue=self.subtitle_ocr_task_queue,
                                                                       progress_queue=self.subtitle_ocr_progress_queue,
                                                                       processed_images=processed_images,
                                                                       use_process=not getattr(sys, 'frozen', False)
                                                                       )
        # 开启线程负责更新OCR进度
        progress_thread = Thread(target=get_ocr_progress, daemon=True)
        progress_thread.start()
        
        # Thêm thread để kiểm tra OCR process có thực sự kết thúc không
        def wait_for_ocr_completion():
            """Đợi OCR process thực sự kết thúc"""
            try:
                # Đợi OCR process kết thúc với timeout
                process.join(timeout=30)  # Đợi tối đa 30 giây
                
                # Kiểm tra xem process có thực sự kết thúc không
                if process.is_alive():
                    print("⚠️ OCR process vẫn đang chạy, đợi thêm...")
                    # Đợi thêm một chút
                    time.sleep(10)
                    if process.is_alive():
                        print("⚠️ OCR process vẫn chạy, có thể có vấn đề")
                        return
                
                # Kiểm tra thêm: OCR có tạo ra file output không
                raw_subtitle_path_normalized = os.path.normpath(self.raw_subtitle_path)
                print(f"🔍 Kiểm tra file OCR output: {raw_subtitle_path_normalized}")
                
                if os.path.exists(raw_subtitle_path_normalized):
                    # Đợi thêm một chút để đảm bảo file được ghi hoàn toàn
                    time.sleep(2)
                    try:
                        with open(raw_subtitle_path_normalized, 'r', encoding='utf-8') as f:
                            content = f.read()
                        if content.strip():  # File có nội dung
                            print("✅ OCR process đã kết thúc hoàn toàn!")
                            # Đảm bảo progress = 100% khi OCR thực sự xong
                            self.update_progress(ocr=100)
                        else:
                            print("⚠️ OCR process kết thúc nhưng file trống")
                            self.update_progress(ocr=100)
                    except FileNotFoundError:
                        # Thư mục hoặc file đã bị xóa (cache cleared), không cần báo lỗi ❌
                        self.update_progress(ocr=100)
                    except Exception as e:
                        # Chỉ báo lỗi nếu là lỗi đọc file thực sự khác
                        print(f"⚠️ Không thể đọc file OCR output: {e}")
                        self.update_progress(ocr=100)
                else:
                    # OCR process kết thúc nhưng không có file (có thể do cache đã bị xóa hoặc chưa có phụ đề)
                    self.update_progress(ocr=100)
            except Exception as e:
                print(f"Lỗi khi đợi OCR process: {e}")
                # Fallback: set progress = 100% nếu có lỗi
                self.update_progress(ocr=100)
        
        Thread(target=wait_for_ocr_completion, daemon=True).start()
        return process
...
if __name__ == '__main__':
    # Tắt multiprocessing để tránh lỗi import
