...
class OcrRecogniser:
    ...
    def _is_gpu_runtime_error(exc):
        ...
        return any(signal in message for signal in gpu_error_signals)

    def _apply_cpu_fallback(self, reason):
        if self._cpu_fallback_applied:
            return

        print('⚠️ OCR GPU khởi tạo thất bại, tự động chuyển sang CPU.')
        print(f'⚠️ Chi tiết lỗi GPU: {reason}')

        os.environ['SUBTITLE_EXTRACTOR_FORCE_CPU'] = '1'
        config.USE_GPU = False

        if hasattr(config, 'ONNX_PROVIDERS'):
            config.ONNX_PROVIDERS = [
                provider for provider in config.ONNX_PROVIDERS if provider != 'CUDAExecutionProvider'
            ]
    ...
    def predict(self, image):
        try:
            ...
            if config.USE_GPU and self._is_gpu_runtime_error(exc):
                self._apply_cpu_fallback(exc)
                self.recogniser = self._build_recogniser()
                detection_box, recognise_result, _ = self.recogniser(image, cls=False)
            else:
                raise
